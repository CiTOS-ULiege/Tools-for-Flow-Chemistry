...
31/08/2024 - reset_input_buffer() replaced to the part of loop timed by change in timestamp (1 Hz frequency)
"""
#comReceivers.py
        ...
        while not stop_event.is_set():  # Check if stop signal is set
            ...
            if rawdata[2:6].isalpha():
                ...
                try:
                    ...
                    continue  # Skip this line if it can't be decoded
                if decoded_data and len(decoded_data) > 6:
                    try:
                        #print("balance - decoded data length > 6, decoded data:", decoded_data)
                        ...
                        if (time.time() - timing1) > 1: #time period for removing data from tuple
                            bal_ser.reset_input_buffer() #restet the serial connection buffer
                            timing1 = time.time()
